...
#        # Post-process patterns to handle z-suffixes
#        # For stitching, we only want to vary the site number, not the z-index
#        # We'll stitch one z-plane at a time
#        for well, channels in patterns_by_well.items():
#            for channel_str, pattern in list(channels.items()):
#                # Check if this well/channel has multiple z-planes
#                has_multiple_z = len(z_indices_by_well_channel[well][channel_str]) > 1
#
#                # For patterns with z-suffixes, we'll create separate patterns for each z-index
#                # But we won't add a {zzz} placeholder - we'll use specific z-indices
#                if has_multiple_z:
#                    # Store the base pattern without z-suffix
#                    base_pattern = re.sub(r'_z\d+', '', pattern)
#
#                    # Create separate patterns for each z-index
#                    for z_index in sorted(z_indices_by_well_channel[well][channel_str]):
#                        # Create a pattern with this specific z-index
#                        z_pattern = base_pattern
#                        ext = os.path.splitext(z_pattern)[1]
#                        z_pattern = z_pattern.replace(ext, f"_z{z_index:03d}{ext}")
#
#                        # Store the pattern with a key that includes the z-index
#                        z_key = f"{channel_str}_z{z_index:03d}"
#                        patterns_by_well[well][z_key] = z_pattern
#
#                    # Remove the original pattern without z-suffix
#                    del patterns_by_well[well][channel_str]
#                else:
#                    # For single z-plane, just remove any z-suffix
#                    pattern = re.sub(r'_z\d+', '', pattern)
#                    patterns_by_well[well][] = pattern
#
#        # Convert defaultdict to regular dict for return
#        result = {well: dict(channels) for well, channels in patterns_by_well.items()}
#
#        # Log the detected patterns
#        for well, channels in result.items():
#            for channel, pattern in channels.items():
#                logger.info(f"Detected pattern for well {well}, channel {channel}: {pattern}")
#
#        return result
