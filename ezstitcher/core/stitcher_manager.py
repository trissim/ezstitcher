...
class StitcherManager:
    ...
    def auto_detect_patterns(folder_path, well_filter=None):
        ...
        for filename in image_files:
            ...
            if match:
                ...
                pattern = filename.replace(f"s{site:03d}", "s{iii}")
    ...
    def create_composite_reference(well, input_dir, processed_dir, reference_channels, channel_patterns, preprocessing_funcs=None, channel_weights=None):
        ...
        # Replace wavelength in pattern with 'composite'
        composite_pattern = re.sub(r'_w\d+', '_wcomposite', first_pattern)
    ...
    @staticmethod
    def ashlar_stitch_v2(image_dir, image_pattern, positions_path, grid_size_x, grid_size_y, tile_overlap=10, tile_overlap_x=None, tile_overlap_y=None, max_shift=20, pixel_size=None):
        """
        ...
        try:
            ...
            overlap_x = tile_overlap_x / 100.0
            overlap_y = tile_overlap_y / 100.0
            
            # Create a FileSeriesReader for the images
            fs_reader = fileseries.FilePatternReader(
                str(image_dir),
                image_pattern,
                overlap_x=overlap_x,
                overlap_y=overlap_y,
                row_major=True,  # Assuming row-major ordering
                grid_shape=(grid_size_y, grid_size_x)  # (rows, cols)
            )
            ...
            with open(positions_path, 'w') as f:
                for i, (filename, position) in enumerate(zip(fs_reader.filenames, mosaic.positions)):
                    # Get grid coordinates
                    grid_y, grid_x = divmod(i, grid_size_x)
                    
                    # Write position to CSV
                    f.write(f"file: {filename}; grid: ({grid_x}, {grid_y}); position: ({position[1]}, {position[0]})\n")
    ...
    def compute_stitched_name(pattern):
        """
        Compute the name for a stitched image based on the pattern.
        
        Args:
            pattern (str): Pattern with '{iii}' placeholder
            
        Returns:
            str: Name for the stitched image
        """
        # Replace site placeholder with 'stitched'
        stitched_name = pattern.replace('s{iii}', 'stitched')
        
        # If pattern doesn't contain the placeholder, add 'stitched' before extension
        if stitched_name == pattern:
            base, ext = os.path.splitext(pattern)
            stitched_name = f"{base}_stitched{ext}"
        
        return stitched_name
    
    @staticmethod
    def process_plate_folder(plate_folder, reference_channels=['1'], preprocessing_funcs=None, margin_ratio=0.1, composite_weights=None, well_filter=None, tile_overlap=6.5, tile_overlap_x=None, tile_overlap_y=None, max_shift=50, focus_detect=False, focus_method="combined", create_projections=False, projection_types=['max', 'mean'], stitch_z_reference='best_focus'):
        """
        ...
        try:
            ...
            if has_zstack:
                ...
                if create_projections:
                    ...
                    projections_dir = parent_dir / f"{plate_name}_projections" / "TimePoint_1"
                    ensure_directory(projections_dir)
                    
                    # Create projections
                    logger.info(f"Creating projections: {projection_types}")
                    projections_created = ZStackManager.create_zstack_projections(
                        dirs['input'],
                        projections_dir,
                        projection_types=projection_types
                    )
                    
                    if not projections_created:
                ...
                if stitch_z_reference == 'best_focus' and best_focus_dir:
                    ...
                    logger.info(f"Stitching each Z-plane separately using stitch_across_z")
                    
                    # Capture the return value from stitch_across_z
                    success = ZStackManager.stitch_across_z(plate_folder, reference_z='all', **{
