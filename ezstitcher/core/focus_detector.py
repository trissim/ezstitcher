"""
Focus detection module for ezstitcher.

This module contains the FocusDetector class for detecting the best focused image in a stack.
"""
...
class FocusDetector:
    """
    Class for detecting the best focused image in a stack.
    """
    ...
    def tenengrad_variance(img, ksize=3, threshold=0):
        ...
        Args:
            ...
            threshold (float): Threshold to reduce noise impact
    ...
    def combined_focus_measure(img, weights=None):
        ...
        nvar = FocusDetector.normalized_variance(img)
        lap = FocusDetector.laplacian_energy(img)
        ten = FocusDetector.tenengrad_variance(img)
        fft = FocusDetector.adaptive_fft_focus(img)
    ...
    def find_best_focus(image_stack, method='combined', roi=None):
        ...
        if method == 'combined':
            focus_func = FocusDetector.combined_focus_measure
        elif method == 'nvar' or method == 'normalized_variance':
            focus_func = FocusDetector.normalized_variance
        elif method == 'lap' or method == 'laplacian':
            focus_func = FocusDetector.laplacian_energy
        elif method == 'ten' or method == 'tenengrad':
            focus_func = FocusDetector.tenengrad_variance
        elif method == 'fft':
            focus_func = FocusDetector.original_fft_focus
        elif method == 'adaptive_fft':
            focus_func = FocusDetector.adaptive_fft_focus
        else:
