#!/usr/bin/env python3
"""
Comprehensive test for ezstitcher using synthetic microscopy data with class-based implementation.

This test:
1. Generates synthetic microscopy data with multiple wavelengths and Z-stacks
2. Tests the core functionality of ezstitcher using the class-based implementation:
...
class TestSyntheticWorkflowClassBased(unittest.TestCase):
    ...
    def test_zstack_projection_creation(self):
        ...
        try:
            ...
            has_zstack, z_info = z_stack_manager.preprocess_plate_folder(self.zstack_dir)
            self.assertTrue(has_zstack, "Failed to detect Z-stack in Z-stack data")

            # Create output directory for projections
            projection_dir = os.path.join(self.test_dir, "synthetic_plate_projections")
            os.makedirs(projection_dir, exist_ok=True)

            # Create projections
            projection_types = ['max', 'mean']
            success, proj_dir = z_stack_manager.create_zstack_projections(
                self.zstack_dir,
    ...
    def test_zstack_detection(self):
        ...
        try:
            ...
            has_zstack, z_info = z_stack_manager.preprocess_plate_folder(self.no_zstack_dir)
    ...
    def test_zstack_best_focus_selection(self):
        ...
        try:
            ...
            # Use the best focus directory path defined in setUp
            best_focus_dir = self.best_focus_dir

            # Select best focus images
            from ezstitcher.core.z_stack_handler import select_best_focus_zstack
            ...
            # Print the list of files in the best focus directory for debugging
            best_focus_files = os.listdir(timepoint_dir)
            print(f"Files found in best focus directory: {best_focus_files}")
